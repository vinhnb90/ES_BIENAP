...
public class DAO extends SqlDAO {
    private static DAO sInstance;

    public static synchronized DAO getInstance(SQLiteDatabase database, Context context) {
        if (sInstance == null)
            sInstance = new DAO(database, context);
        return sInstance;
    }

    private DAO(SQLiteDatabase database, Context context) {
        super(database, context);
    }

    public List<TABLE_DEVICE> getTABLE_DEVICE(String[] agrs) {

        String query = "SELECT * FROM " +
                TABLE_DEVICE.table.getName() +
                " WHERE " +
        ...
        return super.selectAllLazy(TABLE_DEVICE.class, cursor);
    }
    ...
    public List<DeviceAdapter.DataDevice> getDeviceAdapter(Common.MODE mode) {
        ...
        String query = "SELECT * FROM " +
                ...
                mode.content +
                "' ORDER BY " +
                TABLE_DEVICE.table.DEVICE_DATE.name() +
                " DESC";


        Cursor cursor = super.mDatabase.rawQuery(query, null);

        return super.selectCustomLazy(cursor, new ItemFactory(DeviceAdapter.DataDevice.class) {
            @Override
            protected DeviceAdapter.DataDevice create(Cursor cursor, int index) {
                DeviceAdapter.DataDevice dataDevice = new DeviceAdapter.DataDevice();
                cursor.moveToPosition(index);

                dataDevice.setDEVICE_CODE(cursor.getString(cursor.getColumnIndex(TABLE_DEVICE.table.DEVICE_CODE.name())));
                dataDevice.setDEVICE_DATE(cursor.getString(cursor.getColumnIndex(TABLE_DEVICE.table.DEVICE_DATE.name())));
